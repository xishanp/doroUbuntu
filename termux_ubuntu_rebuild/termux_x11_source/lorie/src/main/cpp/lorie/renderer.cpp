...
void Renderer::redrawLocked(bool* waitingForBuffers) {
    ...
    if (state->cursor.updated) {
        ...
        const uint32_t width = state->cursor.width;
        const uint32_t height = state->cursor.height;
        bindTexture(cursor.id);
        // Reset unpack state because root-buffer uploads may leave driver-specific state behind.
        glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
        while (glGetError() != GL_NO_ERROR) {}
        if (width > 0 && height > 0 && width < 512 && height < 512) {
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, (GLsizei) width, (GLsizei) height,
                         0, GL_RGBA, GL_UNSIGNED_BYTE, state->cursor.bits);
            GLenum error = glGetError();
            cursor.cursorChanged = error == GL_NO_ERROR;
            if (error != GL_NO_ERROR)
                loge("Xlorie: cursor upload failed: 0x%x, %ux%u\n", error, width, height);
        } else {
...
void Renderer::drawCursor(float displayWidth, float displayHeight, float sourceLeft, float sourceTop) {
    ...
    if (!state->cursor.width || !state->cursor.height || !cursor.cursorChanged)
        return;
