...
public class LorieView extends SurfaceView implements InputStub {
    ...
    private SurfaceLifecycleListener surfaceLifecycleListener;
    ...
    private final SurfaceHolder.Callback mSurfaceCallback = new SurfaceHolder.Callback() {
        ...
        @Override public void surfaceChanged(@NonNull SurfaceHolder holder, int f, int width, int height) {
            if (surfaceLifecycleListener != null)
                surfaceLifecycleListener.onSurfaceAvailable(holder.getSurface(), width, height);
            else
        ...
        @Override public void surfaceDestroyed(@NonNull SurfaceHolder holder) {
            if (surfaceLifecycleListener != null) surfaceLifecycleListener.onSurfaceDestroyed();
            else LorieView.this.surfaceChanged(null);
        }
    };
    ...
    public void setSurfaceLifecycleListener(SurfaceLifecycleListener listener) {
        surfaceLifecycleListener = listener;
    }

    public void attachRuntimeSurface(Surface surface, int width, int height) {
        surfaceChanged(surface);
        updateViewport();
    }

    public void detachRuntimeSurface() {
        surfaceChanged(null);
    }

    public void applyPreferences() {
        Prefs prefs = InputPreferencesRegistry.get();
        if (prefs != null) reloadPreferences(prefs);
        triggerCallback();
    }
    ...
    private void sendWindowChange() {
        ...
        if (getDisplay() == null || getDisplay().getDisplayId() == Display.DEFAULT_DISPLAY)
            name = "builtin";
        else if (SamsungDexUtils.checkDeXEnabled(getContext()))
            name = "dex";
        else
            name = "external";
    ...
    private static void setRendererViewport(int viewportLeft, int viewportTop, int viewportWidth, int viewportHeight,
        ...
        MainActivity.handler.post(() -> {
            ...
            com.termux.x11.input.InputHost host = InputHostRegistry.get();
            LorieView view = activity != null ? activity.getLorieView() : host != null ? host.view() : null;
            if (view == null) return;
    ...
    public boolean dispatchKeyEventPreIme(KeyEvent event) {
        ...
        MainActivity activity = MainActivity.getInstance();
        if (activity != null) return activity.handleKey(event);
